...
START_TEST(test_BmNet_init)
{
    ...
    BmCode* dep= newBmCode(2, 1, 2);
    BmNet_at_connect(net, 1, dep);
    BmNet_at_connect(net, 2, BmCode_initialize(dep, 3, 2, 3, 4));
    BmNet_at_connect(net, 3, BmCode_initialize(dep, 1, 2));
    BmNet_at_connect(net, 4, BmCode_initialize(dep, 1, 3));
